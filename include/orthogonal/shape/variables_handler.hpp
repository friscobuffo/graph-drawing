...
private:
    ...
    GraphEdgeHashMap<int>& direction_type_to_variable_map(Direction direction, VariableType type);
    void add_variable(int i, int j, Direction direction, VariableType type);
    void add_edge_variables(int i, int j, VariableType type);
    void add_high_degree_node_variables(const Graph& graph, const GraphNode& node);
public:
