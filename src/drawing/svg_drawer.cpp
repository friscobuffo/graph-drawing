...
void SvgDrawer::add(Point2D& point, std::string color, std::string label) {
  ...
  if (!label.empty()) {
    m_svg << "<text x=\"" << point.x + 2 << "\" y=\""
          << m_scaleY.map(point.y + 2) << "\" ";
    m_svg << "font-family=\"Verdana\" font-size=\"16\" fill=\"black\">" << label
          << "</text>" << std::endl;
  }
}
...
void SvgDrawer::add(Polygon2D& polygon, std::string color) {
  ...
  m_svg << "\" style=\"fill:white;stroke:" << color << ";stroke-width:1\" />"
        << std::endl;
  for (Point2D& point : polygon.getPoints()) add(point);
}
