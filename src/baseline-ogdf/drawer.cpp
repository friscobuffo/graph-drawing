...
std::vector<int> count_length_per_edge(
    const ogdf::GraphAttributes &GA, const ogdf::Graph &G, OGDFDrawing &ogdf_drawing
) {
    ...
    for (ogdf::edge e : G.edges)
    {
        ...
        if (ogdf_drawing.is_key_in_edge_to_bend_ids(make_key(source, target))) {
            std::vector<int> bend_ids = ogdf_drawing.get_edge_to_bend_ids(
                make_key(source, target)
            );
            ...
            int x = std::abs(
                std::get<0>(ogdf_drawing.get_id_to_coords(source)) -
                std::get<0>(ogdf_drawing.get_id_to_coords(target))
            );
            int y = std::abs(
                std::get<1>(ogdf_drawing.get_id_to_coords(source)) -
