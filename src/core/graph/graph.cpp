...
const GraphEdge& Graph::add_edge(int from_id, int to_id) {
    ...
    GraphEdge* edge = new GraphEdge(m_next_edge_id++, *from_node, *to_node, *this);
...
const std::any& GraphAttributes::get_attribute_by_id(Attribute attribute, int id) const {
    if (!has_attribute_by_id(attribute, id))
        throw std::runtime_error("GraphAttributes::get_attribute_by_id: does not have attribute");
...
void GraphAttributes::set_edge_any_label(int edge_id, const std::any& label) {
    if (has_attribute_by_id(Attribute::EDGES_ANY_LABEL, edge_id))
        throw std::runtime_error("GraphAttributes::set_edge_any_label: the edge already has a label");
...
const std::any& GraphAttributes::get_edge_any_label(int edge_id) const {
    if (!has_attribute_by_id(Attribute::EDGES_ANY_LABEL, edge_id))
        throw std::runtime_error("GraphAttributes::get_edge_any_label: the edge does not have a label");
