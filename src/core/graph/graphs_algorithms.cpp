...
std::vector<std::vector<int>> compute_all_cycles_in_undirected_graph(
    const Graph& graph) {
...
std::optional<std::vector<int>> find_a_cycle_directed_graph(
    const Graph& graph) {
  std::unordered_map<int, int> state;
...
bool are_cycles_equivalent(const std::vector<int>& cycle1,
