...
void save_cycles_to_file(const std::vector<std::vector<int>>& cycles,
                         const std::string& filename) {
  std::ofstream outfile(filename);
  if (outfile.is_open()) {
...
std::vector<std::vector<int>> load_cycles_from_file(
    const std::string& filename) {
  std::ifstream infile(filename);
  if (infile.is_open()) {
    ...
    while (std::getline(infile, line)) {
      ...
      int node;
      while (iss >> node) cycle.push_back(node);
