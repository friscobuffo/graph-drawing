...
GraphEdgeHashMap<int>& VariablesHandler::direction_type_to_variable_map(Direction direction, VariableType type) {
    switch (direction) {
        case Direction::UP:
            return (type == VariableType::STANDARD) ? m_edge_up_variable : m_special_edge_up_variable;
        case Direction::DOWN:
            return (type == VariableType::STANDARD) ? m_edge_down_variable : m_special_edge_down_variable;
        case Direction::RIGHT:
            return (type == VariableType::STANDARD) ? m_edge_right_variable : m_special_edge_right_variable;
        case Direction::LEFT:
            return (type == VariableType::STANDARD) ? m_edge_left_variable : m_special_edge_left_variable;
        default:
            throw std::invalid_argument("Invalid direction");
    }
}

void VariablesHandler::add_variable(int i, int j, Direction direction, VariableType type) {
    GraphEdgeHashMap<int>& variable_map = direction_type_to_variable_map(direction, type);
...
void VariablesHandler::add_high_degree_node_variables(const Graph& graph, const GraphNode& node) {
...
VariablesHandler::VariablesHandler(const Graph& graph) {
    for (const auto& node : graph.get_nodes()) {
        ...
        if (node.get_degree() > 4)
            add_high_degree_node_variables(graph, node);
    }
}
...
const std::pair<int, int>& VariablesHandler::get_edge_of_variable(int variable) const {
...
int VariablesHandler::get_special_variable(int i, int j, Direction direction) const {
    if (direction == Direction::UP)
        return get_special_up_variable(i, j);
    if (direction == Direction::DOWN)
        return get_special_down_variable(i, j);
    if (direction == Direction::LEFT)
        return get_special_left_variable(i, j);
    if (direction == Direction::RIGHT)
        return get_special_right_variable(i, j);
    throw std::invalid_argument("Invalid direction");
}
