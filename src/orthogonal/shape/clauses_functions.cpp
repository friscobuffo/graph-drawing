...
void add_constraints_one_direction_per_edge(
    ...
    for (auto& node : graph.get_nodes()) {
        ...
        for (auto& edge : node.get_edges()) {
            ...
            add_constraints_one_direction_per_edge(
                graph, cnf_builder, handler, up, down, right, left
            );
        }
    }
}
...
void add_nodes_constraints_special(
    const Graph& graph,
    CnfBuilder& cnf_builder,
    const VariablesHandler& handler
) {
...
void special_implies_same_directions_of_edges(
    VariablesHandler& handler, CnfBuilder& cnf_builder,
    int special_1, int special_2,
    int edge_1_dir_1, int edge_1_dir_2,
    int edge_2_dir_1, int edge_2_dir_2
) {
...
// if the special variable perpendicular is true, then edge1 and edge2 are in the same direction
void add_clauses_special_edges(
    const Graph& graph,
    CnfBuilder& cnf_builder,
    ...
    for (const Direction direction : directions) {
        ...
        int edge_1_var = handler.get_variable(edge_1_from_id, edge_1_to_id, direction);
        int edge_2_var = handler.get_variable(edge_2_from_id, edge_2_to_id, direction);
        int special_var_1 = handler.get_special_variable(
            edge_1_id, edge_2_id, perpendicular_direction
        );
        ...
        cnf_builder.add_clause({-edge_1_var, -edge_2_var, special_var_1, special_var_2});
    }
    ...
    add_constraints_at_most_one_is_true(
        cnf_builder, special_right, special_left, special_up, special_down
    );
...
void add_constraints_special_edges(
    const Graph& graph,
    CnfBuilder& cnf_builder,
    VariablesHandler& handler
) {
    for (auto& node : graph.get_nodes()) {
        ...
        for (int i = 0; i < edges.size()-1; i++) {
            for (int j = i + 1; j < edges.size(); j++) {
                ...
                add_clauses_special_edges(
                    graph, cnf_builder, handler,
                    edge_1_id, edge_2_id
                );
            }
        }
...
void add_cycles_constraints_special(
    const Graph& graph,
    CnfBuilder& cnf_builder,
    const std::vector<std::vector<int>>& cycles,
    const VariablesHandler& handler
    ...
    for (auto& cycle : cycles) {
        ...
        for (int i = 0; i < cycle.size(); i++) {
            ...
            if (edge_1_id > edge_2_id) std::swap(edge_1_id, edge_2_id);
            int special_var_down = handler.get_special_down_variable(edge_1_id, edge_2_id);
            int special_var_up = handler.get_special_up_variable(edge_1_id, edge_2_id);
            int special_var_right = handler.get_special_right_variable(edge_1_id, edge_2_id);
            int special_var_left = handler.get_special_left_variable(edge_1_id, edge_2_id);
