...
Shape result_to_shape(
    ...
    for (int i = 0; i < numbers.size(); i++) {
        int var = numbers[i];
        if (var > 0) handler.set_variable_value(var, true);
        else handler.set_variable_value(-var, false);
    }
    Shape shape;
...
Shape build_shape_special(
    ...
    int seed = (randomize) ? std::random_device{}() : 42;
    std::mt19937 random_engine(seed);
    auto shape = build_shape_or_add_corner_special(graph, attributes, cycles, random_engine);
    while (!shape.has_value())
        shape = build_shape_or_add_corner_special(graph, attributes, cycles, random_engine);
    return std::move(shape.value());
}
